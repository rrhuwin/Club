...
class Pageinfo(object):
    ...
    def a_page(self):
        ...
        if self.page <= 1:
            ...
            prev = f'<li class="page-item "><a data-pjax="true" class="page-link" href="#" aria-label="Previous"><span aria-hidden="true">&laquo;</span><span class="sr-only">Previous</span></a></li>'
        else:
            # prev = f"<a class='page-link' href='{self.url}?page={self.page - 1}' aria-label='Previous'><< 上一页</a>"
            prev = f'<li class="page-item "><a data-pjax="true" class="page-link" href="{self.url}{self.tp}/{self.page - 1}" aria-label="Previous"><span aria-hidden="true">&laquo;</span><span class="sr-only">Previous</span></a></li>'
        if self.page >= self.all_page:
            next = f"<a class='page-link' href='' aria-label='Next' data-pjax='true'>下一页>></a>"
            ...
            next = f'<li class="page-item"><a class="page-link" data-pjax="true" href="{self.url}{self.tp}/{self.page + 1}" aria-label="Next"><span aria-hidden="true">&raquo;</span><span class="sr-only">Next</span></a></li>'
        ...
        for i in range(begin, sotp):
            if i == self.page:
                ...
                v= f'<li class="page-item active"><a class="page-link" data-pjax="true" href="{self.url}{self.tp}/{i}">{i}</a></li>'
            else:
                # v = f"<a  class='page-item active' href='{self.url}?page={i}'>{i}</a>"
                v= f'<li class="page-item "><a class="page-link" data-pjax="true" href="{self.url}{self.tp}/{i}">{i}</a></li>'
